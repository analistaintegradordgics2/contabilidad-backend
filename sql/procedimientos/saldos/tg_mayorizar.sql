-- DROP FUNCTION public.tg_mayorizar();

CREATE OR REPLACE FUNCTION public.tg_mayorizar()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
...
BEGIN
	...
	if lnidestado = 1 then
		-- Declaramos los Saldos en 0
		-- Vamos  a buscar la cantidad del mayor_id para empezar a hacer la jerarquia
		select  length(rtrim(zz.codigo)), rtrim(zz.codigo) into lnidJerarquia, lccodigoJer  from contabilidad_mayor zz where zz.id = lnidmayor;
		WHILE lnidJerarquia >0 loop
			lnd01   := 0.00;
			lnh01   := 0.00;
			lnsal01 := 0.00;
			lnd02   := 0.00;
			...
			select COALESCE(aa.d01,0.00), COALESCE(aa.h01,0.00), COALESCE(aa.sal01,0.00),
				   ...
				   lnd12, lnh12, lnsal12,
				   lnd13, lnh13, lnsal13,
				   lnidtabla from contabilidad_saldos aa where aa.mayor_id =lnidmayor and aa.anio = lnaño ;
			select  count(*) into  lncontsaldo from contabilidad_saldos ac where ac.mayor_id = lnidmayor and ac.anio = lnaño; 
			...
			if lncontsaldo = 0 then
				Insert into contabilidad_saldos ( anio, mayor_id, cont_mayor_niif_id, sali,
										  d01, h01, sal01,
										  d02, h02, sal02,
										  d03, h03, sal03,
										  d04, h04, sal04,
				...
				update contabilidad_saldos set 
				...
				if lnidJerarquia = 1 then
					lnidJerarquia := 0 ;
				end if ;
				
				if lnidjerarquia = 2 then
					lnidJerarquia := 1 ;
				end if ;
				
				if lnidjerarquia >3 then
					lnIdjerarquia := lnidjerarquia -2 ;
				end if ;

				if lnidjerarquia>0 then
					lccodigoJer := substring(lccodigoJer,1,lnidjerarquia) ;
					select  ad.id into lnidmayor from contabilidad_mayor ad where ad.codigo = lccodigoJer ;
				end if;
			end	if ;	
		end loop ;
	end if ;
...
DROP TRIGGER IF EXISTS trg_mayorizar ON contabilidad_mov;
CREATE TRIGGER trg_mayorizar
    AFTER INSERT OR UPDATE ON contabilidad_mov
    FOR EACH ROW
    EXECUTE FUNCTION public.tg_mayorizar();
