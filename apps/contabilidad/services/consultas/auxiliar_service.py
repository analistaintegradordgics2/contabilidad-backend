...
from .consultas_auxliares import *
import os, shutil
...
class ConsultaService:
    ...
    CONSULTAS_AUXILIARES = {
        1: ConsultaCodigoPorNit(),
        2: ConsultaCodigoYNit(),
        3: ConsultaCodigo(),
        4: ConsultaNitCuenta(),
        5: ConsultaNit(),
        7: ConsultaCodigoNitConcepto(),
    }

    @staticmethod
    def filtro_aux(model, filtro=None):
        consulta = ConsultaService.CONSULTAS_AUXILIARES[model['tipo']]
        if not consulta:
            raise ValueError(f"El tipo de consulta {model['tipo']} no existe.")
        return consulta.ejecutar(model, filtro)
